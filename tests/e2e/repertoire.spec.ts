...
test("repertoire page renders without console warnings", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "Untitled Repertoire" })).toHaveCount(0);
...
test("creates a chapter from the top bar before training", async ({ page }) => {
  const consoleMessages = collectUnexpectedConsole(page);
  ...
  const createChapterBox = await createChapterButton.boundingBox();
  const trainBox = await trainLink.boundingBox();
  if (createChapterBox === null || trainBox === null) {
    throw new Error("Expected create chapter and Train controls to have visible boxes");
  }
  expect(createChapterBox.x).toBeLessThan(trainBox.x);

  await createChapterButton.click();
  await page.getByText("Create empty chapter").click();
